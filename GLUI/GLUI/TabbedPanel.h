...
namespace GLUI {
	...
	void TabbedPanel::draw(bool draw_background) {
		...
		for (int i = 0; i < btn_selectors.size(); ++i) {																// Calculate tab selector buttons' positions
			...
			if (i == this->selected) {																					// Highlight the selected tab's title
				...
				btn_selectors[i]->set_background_color(Color(120, 120, 120, 255));
			}
		}
	...
	void TabbedPanel::action_performed(void* sender, Event& e) {
		for (int i = 0; i < btn_selectors.size(); ++i) {																// Iterate through the tab selector buttns
			if (sender == btn_selectors[i]) {																			// To find wich one was pressed
	...
	TabbedPanel::TabbedPanel(float x, float y, float width, float height, float border_width) : Panel(true, x, y, width, height, border_width) {
		...
		this->pnl_main_tab->set_background_color(255, 255, 255, 0);
	...
	void TabbedPanel::remove_tab(std::string tab_title) {
		...
		if (id != -1) {
			btn_selectors[id]->remove_event_listener(this);
