...
namespace GLUI {
	...
	void Button::handle_event(Event& e) {
		...
		if (e.mouse_is_inside && !e.mouse_is_covered) {																						// If the mouse is above the button and check if covered, because one does not simply press a button if it's behind a component
			...
			if (e.mouse_pressed && e.mouse_left && !this->pressed) {																		// And if the user pressed the left mouse button
				...
				if (this->watch_wait.get_elapsed<Time::SECONDS>() > Duration<Time::SECONDS>(this->wait_time - this->repeat_time).count()) {	// And if enough time has elapsed to start repeating button presses
					if (!this->watch_repeat.is_running()) {																					// And if repeat watch is not running
						this->watch_repeat.start();																							// Start it
					} else {																												// Else
						if (this->watch_repeat.get_elapsed<Time::SECONDS>() > Duration<Time::SECONDS>(this->repeat_time).count()) {			// If enough time has elapsed after the last repeated button event to start new ones
	...
	void Button::draw(bool draw_background) {
		this->lbl_text->set_position(float2(this->default_border_width, this->default_border_width));	// Dynamically set the label's position to match the button's position
		this->lbl_text->set_size(this->get_size() -
								 float2(this->default_border_width, this->default_border_width));		// Dynamically set the label's size to match the button's size
	...
	Button::Button(std::string text, float x, float y, float width, float height, float border_width) : Component(x, y, width, height, border_width) {
		this->wait_time = Duration<Time::MILLI_SECONDS>(500);		// Default 500 ms until repeating button presses
		this->repeat_time = Duration<Time::MILLI_SECONDS>(100);		// Default 100 ms between repeated button pressed
	...
	// Sets the wait time
	template<typename Time_TYPE> void Button::set_wait_time(Duration<Time_TYPE> wait_time) {
		this->wait_time = wait_time;
	}

	// Sets the repeat time
	template<typename Time_TYPE> void Button::set_repeat_time(Duration<Time_TYPE> repeat_time) {
		this->repeat_time = repeat_time;
	}

	// Gets the wait time
	Duration<Time::SECONDS> Button::get_wait_time() {
		return this->wait_time;
	}

	// Gets the repeat time
	Duration<Time::SECONDS> Button::get_repeat_time() {
